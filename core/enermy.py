...
class Enermy :
	...
	def render (self, frame, cam) :
		...
		if self.deathAnim < dethAnimSize :
			img = pg.transform.rotate(
					pg.transform.scale(
						self.deathRect,
						(
							dethAnimSize - self.deathAnim,
							self.deathAnim
						)
					),
					self.deathAnimRot)
	...
	def update (self, level, player) :
		...
		if self.dead and self.deathAnim > 0 and self.deathAnim < dethAnimSize : self.deathAnim *= 0.6